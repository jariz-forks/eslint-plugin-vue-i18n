...
function objectExpressionToDict(node) {
    var _a;
    const dict = {};
    for (const prop of node.properties) {
        ...
        if (value.type === 'ObjectExpression') {
            ...
            dict[key] = (_a = value.quasis[0].value.cooked) !== null && _a !== void 0 ? _a : value.quasis[0].value.raw;
        }
    }
    return dict;
}
function collectVariableDeclarations(ast) {
    const map = new Map();
...
function resolveMessagesValue(node, variableMap, importMap, filename) {
    ...
    if (resolved.type === 'Identifier') {
        ...
        if (importInfo) {
            ...
            const fullpath = (0, path_1.resolve)(dir, importInfo.source);
...
function extractUseI18nMessages(ast, filename, visitorKeys) {
    ...
    vue_eslint_parser_1.AST.traverseNodes(ast, {
        ...
        enterNode(node) {
            ...
            for (const prop of arg.properties) {
                ...
                if (key !== 'messages')
                    continue;
                const resolved = resolveMessagesValue(prop.value, variableMap, importMap, filename);
                if (!resolved)
                    break;
                if (resolved.type === 'inline') {
                    results.push(new locale_messages_1.UseI18nLocaleMessage({
                        fullpath: filename,
                        messages: resolved.dict
                    }));
                }
                else {
                    results.push(new locale_messages_1.FileLocaleMessage({
...
function getLocaleMessagesFromUseI18n(context) {
    const sourceCode = (0, compat_1.getSourceCode)(context);
    let results = useI18nLocaleMessagesCache.get(sourceCode.ast);
    if (results)
        return results;
    const filename = (0, compat_1.getFilename)(context);
    results = extractUseI18nMessages(sourceCode.ast, filename, sourceCode.visitorKeys);
